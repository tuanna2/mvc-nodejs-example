(function ($) {
    ...
    function createOption(data) {
        ...
        addItem({ name: "code", title: "Mã đặt", type: "text" });
        addItem({ name: "branch_id", title: "Hãng", type: "select", items: [{ id: 0, name: "" }].concat(data.branchs), valueField: "id", textField: "name", width: 50 });
        
        addItem({ name: "order_date", title: "Ngày đặt", type: "text", width: 50 });
        addItem({ name: "deliver_date", title: "Ngày trả", type: "text", width: 50 });
        addItem({ name: "received_date", title: "Ngày nhận", type: "text", width: 50 });

        addItem({ name: "status", title: "Trạng thái", type: "select", items: [{ id: 0, name: "" }].concat(data.status), valueField: "id", textField: "name", itemTemplate: getBadgeItem, width: 50 });
        addItem({ name: "total", title: "Tổng tiền", type: "text", width: 50 });
    ...
    $(function () {
        $.get('/api/branchs', function (res) {
            ...
            $.get('/api/request-status', function (res) {
                status = res.data;
                initGrid("#js-grid-order", {branchs: branchs, status: status});
            })
        })
    });
})(jQuery);
